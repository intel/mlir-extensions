...
public:
  ...
  LogicalResult matchAndRewrite(scf::ParallelOp op,
    ...
    // Temporary disable if contains induction variables, it's not clear for now what is to do with those inductions
    ...
    for (auto s : op.step()) {
      if (auto c = s.getDefiningOp<ConstantIndexOp>()) {
        ...
        return rewriter.notifyMatchFailure(op, "scf.parallel->affine.parallel non constant step");
      }
    }
    ...
    if (llvm::any_of(op.region().getOps(), [&](const Operation& each) {
      ...
      return rewriter.notifyMatchFailure(op, "scf.parallel->affine.parallel reduction is detected");
    }
