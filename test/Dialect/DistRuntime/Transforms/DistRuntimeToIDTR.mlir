...
// -----
module {
  func.func @test_copy_reshape(%arg0: tensor<?x?xi64>) -> tensor<3xi64> {
    %c1 = arith.constant 1 : index
    %c3 = arith.constant 3 : index
    %c9 = arith.constant 9 : index
    %handle, %nlArray = distruntime.copy_reshape %arg0 g_shape %c3, %c3 l_offs %c1, %c1 to n_g_shape %c9 n_offs %c3 n_shape %c3 {team = 22 : i64} : (tensor<?x?xi64>, index, index, index, index, index, index, index) -> (!distruntime.asynchandle, tensor<3xi64>)
    "distruntime.wait"(%handle) : (!distruntime.asynchandle) -> ()
    return %nlArray : tensor<3xi64>
  }
}
...
// CHECK: [[V0:%.*]] = ndarray.create %c3 : (index) -> tensor<3xi64>
// CHECK: [[handle:%.*]] = call @_idtr_copy_reshape_i64
// CHECK: call @_idtr_wait([[handle]]) : (i64) -> ()
// CHECK: return [[V0]] : tensor<3xi64>

// -----
module {
  func.func @test_copy_permute(%arg0: tensor<5x2xi64>) -> tensor<2x5xi64> {
    ...
    return %a : tensor<2x5xi64>
  }
}
// CHECK-LABEL: func.func @test_copy_permute
// CHECK: [[V0:%.*]] = ndarray.create %c2, %c5 : (index, index) -> tensor<2x5xi64>
// CHECK: [[handle:%.*]] = call @_idtr_copy_permute_i64
// CHECK: call @_idtr_wait([[handle]]) : (i64) -> ()
// CHECK: return [[V0]] : tensor<2x5xi64>
