...
// CHECK-LABEL: func @test_gemm_vc({{.*}}) {
func.func @test_gemm_vc(%a : memref<1024x1024xf16>, %b: memref<1024x1024xf16>, %c: memref<1024x1024xf32>) {
  ...
  scf.for %i= %c0 to %c1024 step %c8 {
    scf.for %j= %c0 to %c1024 step %c16 {
      ...
      %1 = xegpu.create_nd_tdesc %a[%i, %c0] {mode = vc} : memref<1024x1024xf16> -> !xegpu.tensor_desc<8x16xf16>

      // CHECK: xegpu.create_nd_tdesc
      // CHECK-SAME: memref<1024x1024xf16> -> !xegpu.tensor_desc<16x16xf16>
      %2 = xegpu.create_nd_tdesc %b[%c0, %j] {mode = vc} : memref<1024x1024xf16> -> !xegpu.tensor_desc<16x16xf16>
      ...
      %tmp0, %tmp1, %result = scf.for %k= %c0 to %c1024 step %c16
        ...
        %4 = xegpu.load_nd %subA {mode = vc, vnni_axis = 1} : !xegpu.tensor_desc<8x16xf16> -> vector<8x8x2xf16>

        // CHECK: xegpu.load_nd
        // CHECK-SAME: !xegpu.tensor_desc<16x16xf16> -> vector<8x16x2xf16>
        %5 = xegpu.load_nd %subB {mode = vc, vnni_axis = 0} : !xegpu.tensor_desc<16x16xf16> -> vector<8x16x2xf16>

        // CHECK: xegpu.dpas
        // CHECK-SAME: vector<8x8x2xf16>, vector<8x16x2xf16>, vector<8x16xf32> -> vector<8x16xf32>
        %6 = xegpu.dpas %4, %5, %subC {mode = vc} : vector<8x8x2xf16>, vector<8x16x2xf16>, vector<8x16xf32> -> vector<8x16xf32>

        %7 = xegpu.update_nd_offset %subA, [%c0, %c16] {mode = vc} : !xegpu.tensor_desc<8x16xf16> -> !xegpu.tensor_desc<8x16xf16>

        %8 = xegpu.update_nd_offset %subB, [%c16, %c0] {mode = vc} : !xegpu.tensor_desc<16x16xf16> -> !xegpu.tensor_desc<16x16xf16>
