  ...
  scf.for %i= %c0 to %c1024 step %c8 {
    scf.for %j= %c0 to %c1024 step %c16 {
      ...
      %tmp0, %tmp1, %result = scf.for %k= %c0 to %c1024 step %c16 iter_args(%subA = %1, %subB = %2, %subC = %3)
        ...
        %4 = xegpu.load_nd %subA : !xegpu.tensor_desc<8x16xbf16, #sg_map_fp16_a> -> vector<4x2xbf16>

        // CHECK: xegpu.load_nd
        // CHECK-SAME: vector<8x1x2xbf16>
        %5 = xegpu.load_nd %subB {packed} : !xegpu.tensor_desc<16x16xbf16, #sg_map_fp16_b> -> vector<8x1x2xbf16>

        // CHECK: xegpu.dpas
        // CHECK-SAME: vector<4x1x2xbf16>, vector<8x1x2xbf16>, vector<8x1xf32> -> vector<8x1xf32>
        %6 = xegpu.dpas %4, %5, %subC  : vector<4x2xbf16>, vector<8x1x2xbf16>, vector<8x1xf32> -> vector<8x1xf32>
