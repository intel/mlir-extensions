// RUN: imex-opt --split-input-file --convert-xetile-to-xegpu --cse %s -verify-diagnostics -o -| FileCheck %s
  gpu.module @test_kernel {
    ...
    gpu.func @sg_scf_for(%arg0: memref<1024x1024xf16>, %arg1: memref<1024x1024xf16>) {
      ...
      // CHECK-SAME: iter_args(%[[arg3:.*]] = %[[R0]], %[[arg4:.*]] = %[[cst]]) -> (!xegpu.tensor_desc<32x32xf16>, vector<32x32xf16>)
      ...
      // CHECK: %[[R2:.*]] = xegpu.create_nd_tdesc %[[ARG1]][%[[c0]], %[[c64]]] {mode = vc} : memref<1024x1024xf16> -> !xegpu.tensor_desc<8x32xf16>
      // CHECK: %[[c8:.*]] = arith.constant 8 : index
      // CHECK: %[[R3:.*]] = xegpu.create_nd_tdesc %[[ARG1]][%[[c8]], %[[c64]]] {mode = vc} : memref<1024x1024xf16> -> !xegpu.tensor_desc<8x32xf16>
      // CHECK: %[[c16:.*]] = arith.constant 16 : index
      // CHECK: %[[R4:.*]] = xegpu.create_nd_tdesc %[[ARG1]][%[[c16]], %[[c64]]] {mode = vc} : memref<1024x1024xf16> -> !xegpu.tensor_desc<8x32xf16>
      // CHECK: %[[c24:.*]] = arith.constant 24 : index
      // CHECK: %[[R5:.*]] = xegpu.create_nd_tdesc %[[ARG1]][%[[c24]], %[[c64]]] {mode = vc} : memref<1024x1024xf16> -> !xegpu.tensor_desc<8x32xf16>
      ...
      // CHECK: xegpu.store_nd %[[R6]], %[[R2]] {mode = vc, l1_hint = write_back, l2_hint = write_back, l3_hint = write_back} : vector<8x32xf16>, !xegpu.tensor_desc<8x32xf16>
      // CHECK: xegpu.store_nd %[[R7]], %[[R3]] {mode = vc, l1_hint = write_back, l2_hint = write_back, l3_hint = write_back} : vector<8x32xf16>, !xegpu.tensor_desc<8x32xf16>
      // CHECK: xegpu.store_nd %[[R8]], %[[R4]] {mode = vc, l1_hint = write_back, l2_hint = write_back, l3_hint = write_back} : vector<8x32xf16>, !xegpu.tensor_desc<8x32xf16>
      // CHECK: xegpu.store_nd %[[R9]], %[[R5]] {mode = vc, l1_hint = write_back, l2_hint = write_back, l3_hint = write_back} : vector<8x32xf16>, !xegpu.tensor_desc<8x32xf16>
