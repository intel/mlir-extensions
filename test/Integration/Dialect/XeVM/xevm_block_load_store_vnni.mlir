// RUN: %python_executable %imex_runner --requires=l0-runtime -i %s --pass-pipeline-file=%p/xevm-to-llvm.pp \
...
module @gemm attributes {gpu.container_module} {
  gpu.module @kernel {
    gpu.func @block_load_store(%src: !llvm.ptr<1>, %dst: !llvm.ptr<1>) kernel  {
      ...
      // Normal load:
      %loaded = xevm.blockload2d %src, %base_width, %base_height_load, %base_pitch, %x, %y {elem_size_in_bits=16, tile_width=16, tile_height=16, v_blocks=1, transpose=false, vnni_transform=false, l1_cache_control=Default, l3_cache_control=Default} : (!llvm.ptr<1>, i32, i32, i32, i32, i32) -> vector<16xi16>
      ...
      // VNNI load:
      %loaded_vnni = xevm.blockload2d %src, %base_width, %base_height_load, %base_pitch, %x, %y {elem_size_in_bits=16, tile_width=16, tile_height=16, v_blocks=1, transpose=false, vnni_transform=true, l1_cache_control=Default, l3_cache_control=Default} : (!llvm.ptr<1>, i32, i32, i32, i32, i32) -> vector<8xi32>
      %loaded_vnni_f16_flat = vector.bitcast %loaded_vnni : vector<8xi32> to vector<16xf16>
      %loaded_vnni_f16 = vector.shape_cast %loaded_vnni_f16_flat : vector<16xf16> to vector<8x1x2xf16>
      ...
      // VNNI load "packs" multiple sub-dword values along the column (↓), allowing a single register to hold multiple values.
      //  In SIMT, a work-item reads values along the column (↓), hence a sequence of values loaded using VNNI is logically equivalent to the sequence of values loaded using a normal load.
      ...
      %loaded_f16_modified = vector.insert %thread_x_f16, %loaded_vnni_f16 [0,0,1] : f16 into vector<8x1x2xf16> // Both loaded_vnni_f16 and loaded_f16 can be used here
      ...
      xevm.blockstore2d %dst, %base_width_store, %base_height_store, %base_pitch_store, %x, %y, %loaded_f16_modified_slice_0_flat {elem_size_in_bits=16, tile_width=16, tile_height=8, v_blocks=1, l1_cache_control=Default, l3_cache_control=Default} : (!llvm.ptr<1>, i32, i32, i32, i32, i32, vector<8xf16>)
      ...
      xevm.blockstore2d %second_half_ptr, %base_width_store, %base_height_store, %base_pitch_store, %x, %y, %loaded_f16_modified_slice_1_flat {elem_size_in_bits=16, tile_width=16, tile_height=8, v_blocks=1, l1_cache_control=Default, l3_cache_control=Default} : (!llvm.ptr<1>, i32, i32, i32, i32, i32, vector<8xf16>)
      gpu.return
    }
  }
  ...
  func.func @test(%src : memref<16x16xf16>) -> memref<16x16xf16> attributes {llvm.emit_c_interface} {
    ...
    %memref_src = gpu.alloc host_shared () : memref<16x16xf16>
    memref.copy %src, %memref_src : memref<16x16xf16> to memref<16x16xf16>
    ...
    %memref_dst = gpu.alloc host_shared () : memref<16x16xf16>
    ...
    return %memref_dst : memref<16x16xf16>
  }
