...
module @gemm attributes {gpu.container_module} {
  ...
  gpu.module @test_kernel attributes {spirv.target_env = #spirv.target_env<#spirv.vce<v1.4, [Addresses, Float16Buffer, Int64, Int16, Int8, Kernel, Linkage, Vector16, GenericPointer, Groups, Float16, Float64, AtomicFloat32AddEXT, ExpectAssumeKHR, SubgroupDispatch, VectorComputeINTEL, VectorAnyINTEL], [SPV_EXT_shader_atomic_float_add, SPV_KHR_expect_assume, SPV_INTEL_vector_compute]>, api=OpenCL, #spirv.resource_limits<>>} {
    gpu.func @test_kernel(%A: memref<8x32xf16>, %B: memref<16x32xf16>, %Out: memref<8x16xf32>) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      ...
      %a_tile0 = xegpu.create_nd_tdesc %A [%c0, %c0] { mode = vc } : memref<8x32xf16> -> !xegpu.tensor_desc<8x16xf16>
      %a_tile1 = xegpu.create_nd_tdesc %A [%c0, %c16] { mode = vc } : memref<8x32xf16> -> !xegpu.tensor_desc<8x16xf16>
      // load A tiles
      %val0 = xegpu.load_nd %a_tile0 { mode = vc, vnni_axis = 1} : !xegpu.tensor_desc<8x16xf16> -> vector<8x8x2xf16>
      %val1 = xegpu.load_nd %a_tile1 { mode = vc, vnni_axis = 1} : !xegpu.tensor_desc<8x16xf16> -> vector<8x8x2xf16>
      %b_tile0 = xegpu.create_nd_tdesc %B [%c0, %c0] { mode = vc } : memref<16x32xf16> -> !xegpu.tensor_desc<16x16xf16>
      %b_tile1 = xegpu.create_nd_tdesc %B [%c0, %c16] { mode = vc } : memref<16x32xf16> -> !xegpu.tensor_desc<16x16xf16>
      // load B tiles
      %val2 = xegpu.load_nd %b_tile0 { mode = vc, vnni_axis = 0} : !xegpu.tensor_desc<16x16xf16> -> vector<8x16x2xf16>
      %val3 = xegpu.load_nd %b_tile1 { mode = vc, vnni_axis = 0} : !xegpu.tensor_desc<16x16xf16> -> vector<8x16x2xf16>
      // do DPAS
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %B_ = memref.collapse_shape %B [[0, 1]] :memref<16x32xf16> into memref<512xf16>
    %A_random = memref.cast %A_ : memref<256xf16> to memref<?xf16>
    %B_random = memref.cast %B_ : memref<512xf16> to memref<?xf16>
