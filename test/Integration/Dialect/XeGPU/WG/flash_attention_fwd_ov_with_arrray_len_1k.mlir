...
module @fragment_name attributes {gpu.container_module} {
  func.func @entry(%arg0: memref<16x1024x64xf16>, %arg1: memref<16x1024x64xf16>, %arg2: memref<16x1024x64xf16>, %arg3: memref<16x1024x64xf16>) attributes {gc.num_kernels = 1 : i32} {
    %c1 = arith.constant 1 : index
    %c128 = arith.constant 128 : index
    %c16 = arith.constant 16 : index
    %c8 = arith.constant 8 : index
    gpu.launch_func  @entry_kernel::@entry_kernel blocks in (%c16, %c8, %c1) threads in (%c128, %c1, %c1)  args(%arg0 : memref<16x1024x64xf16>, %arg1 : memref<16x1024x64xf16>, %arg2 : memref<16x1024x64xf16>, %arg3 : memref<16x1024x64xf16>)
    return
  }
  gpu.module @entry_kernel [#xevm.target<O = 3>, #xevm.target<flags = {"cmd-options" = ["-ze-opt-large-register-file"]}>] {
  gpu.func @entry_kernel(%Q: memref<16x1024x64xf16>, %K: memref<16x1024x64xf16>, %V: memref<16x1024x64xf16>, %Out: memref<16x1024x64xf16>) kernel attributes {known_block_size = array<i32: 128, 1, 1>} {
    ...
    // Load Q tile
    %q_base_buffer, %q_offset, %q_sizes:3, %q_strides:3 = memref.extract_strided_metadata %Q : memref<16x1024x64xf16> -> memref<f16>, index, index, index, index, index, index, index
    %c65536 = arith.constant 65536 : index
    %q_offset_bh = arith.muli %block_id_x, %c65536 overflow<nsw> : index
    ...
    %k_base_buffer, %k_offset_meta, %k_sizes:3, %k_strides:3 = memref.extract_strided_metadata %K : memref<16x1024x64xf16> -> memref<f16>, index, index, index, index, index, index, index
    %k_intptr = memref.extract_aligned_pointer_as_index %k_base_buffer : memref<f16> -> index
    %c65536_kv = arith.constant 65536 : index
    %kv_offset_bh = arith.muli %block_id_x, %c65536_kv overflow<nsw> : index
    ...
    %v_base_buffer, %v_offset_meta, %v_sizes:3, %v_strides:3 = memref.extract_strided_metadata %V : memref<16x1024x64xf16> -> memref<f16>, index, index, index, index, index, index, index
    ...
    %result:3 = scf.for %k_offset = %c0 to %c1024 step %c64 iter_args(%acc_in = %zero_acc, %m_i_in = %m_i_init, %l_i_in = %l_i_init) -> (vector<128x64xf32>, vector<128xf32>, vector<128xf32>) {
      // Prefetch next-iteration K tile
      %k_next_offset = arith.addi %k_offset, %c64 : index
      %c65536_next = arith.constant 65536 : index
      %kv_next_offset_bh = arith.muli %block_id_x, %c65536_next overflow<nsw> : index
      ...
      %c65536_curr = arith.constant 65536 : index
      %kv_curr_offset_bh = arith.muli %block_id_x, %c65536_curr overflow<nsw> : index
    ...
    // Store output
    %o_base_buffer, %o_offset_meta, %o_sizes:3, %o_strides:3 = memref.extract_strided_metadata %Out : memref<16x1024x64xf16> -> memref<f16>, index, index, index, index, index, index, index
  ...
  func.func @gpu_impl(%arg0: memref<16x1024x64xf16>, %arg1: memref<16x1024x64xf16>, %arg2: memref<16x1024x64xf16>, %arg3: memref<16x1024x64xf16>) -> memref<16x1024x64xf16> {
    // Allocate GPU buffers
    %memref = gpu.alloc  () : memref<16x1024x64xf16>
    %memref_0 = gpu.alloc  () : memref<16x1024x64xf16>
    %memref_1 = gpu.alloc  () : memref<16x1024x64xf16>
    %memref_2 = gpu.alloc  () : memref<16x1024x64xf16>

    // Copy from CPU to GPU
    gpu.memcpy  %memref, %arg0 : memref<16x1024x64xf16>, memref<16x1024x64xf16>
    gpu.memcpy  %memref_0, %arg1 : memref<16x1024x64xf16>, memref<16x1024x64xf16>
    gpu.memcpy  %memref_1, %arg2 : memref<16x1024x64xf16>, memref<16x1024x64xf16>

    // Call the entry function which launches the kernel
    call @entry(%memref, %memref_0, %memref_1, %memref_2) : (memref<16x1024x64xf16>, memref<16x1024x64xf16>, memref<16x1024x64xf16>, memref<16x1024x64xf16>) -> ()

    // Wait for GPU to finish
    gpu.wait

    // Copy output back to CPU
    gpu.memcpy  %arg3, %memref_2 : memref<16x1024x64xf16>, memref<16x1024x64xf16>

    // Cleanup GPU buffers
    gpu.dealloc  %memref : memref<16x1024x64xf16>
    gpu.dealloc  %memref_0 : memref<16x1024x64xf16>
    gpu.dealloc  %memref_1 : memref<16x1024x64xf16>
    gpu.dealloc  %memref_2 : memref<16x1024x64xf16>
    return %arg3 : memref<16x1024x64xf16>
  }
  func.func @cpu_impl(%arg0: memref<16x1024x64xf16>, %arg1: memref<16x1024x64xf16>, %arg2: memref<16x1024x64xf16>, %arg3: memref<16x1024x64xf16>, %arg4: f32) -> memref<16x1024x64xf16> {
    ...
    %alloc = memref.alloc(%c1024, %c1024) : memref<?x?xf32>
    ...
    scf.for %arg5 = %c0 to %c16 step %c1 {
      ...
      scf.for %arg6 = %c0 to %c1024 step %c1 {
        scf.for %arg7 = %c0 to %c1024 step %c1 {
          %0 = scf.for %arg8 = %c0 to %c64 step %c1 iter_args(%arg9 = %cst_0) -> (f32) {
            %2 = memref.load %arg0[%arg5, %arg6, %arg8] : memref<16x1024x64xf16>
            %3 = memref.load %arg1[%arg5, %arg7, %arg8] : memref<16x1024x64xf16>
      ...
      scf.for %arg6 = %c0 to %c1024 step %c1 {
        // Max reduce
        %0 = scf.for %arg7 = %c0 to %c1024 step %c1 iter_args(%arg8 = %cst) -> (f32) {
        ...
        // Take sum of row
        %1 = scf.for %arg7 = %c0 to %c1024 step %c1 iter_args(%arg8 = %cst_0) -> (f32) {
          %2 = memref.load %alloc[%arg6, %arg7] : memref<?x?xf32>
      ...
      scf.for %arg6 = %c0 to %c1024 step %c1 {
        scf.for %arg7 = %c0 to %c64 step %c1 {
          %0 = scf.for %arg8 = %c0 to %c1024 step %c1 iter_args(%arg9 = %cst_0) -> (f32) {
            %2 = memref.load %alloc[%arg6, %arg8] : memref<?x?xf32>
            %3 = memref.load %arg2[%arg5, %arg8, %arg7] : memref<16x1024x64xf16>
          ...
          memref.store %1, %arg3[%arg5, %arg6, %arg7] : memref<16x1024x64xf16>
        }
      }
    }
  ...
  func.func @init_3d_memref_to_const_f16(%arg0: memref<16x1024x64xf16>, %arg1: f16) {
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c16 = arith.constant 16 : index
    %c1024 = arith.constant 1024 : index
    ...
    scf.for %arg2 = %c0 to %c16 step %c1 {
      scf.for %arg3 = %c0 to %c1024 step %c1 {
        scf.for %arg4 = %c0 to %c64 step %c1 {
          memref.store %arg1, %arg0[%arg2, %arg3, %arg4] : memref<16x1024x64xf16>
        }
      }
    }
    return
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %alloc = memref.alloc() : memref<16x1024x64xf16>
    %alloc_3 = memref.alloc() : memref<16x1024x64xf16>
    %alloc_4 = memref.alloc() : memref<16x1024x64xf16>
    %alloc_5 = memref.alloc() : memref<16x1024x64xf16>
    %alloc_6 = memref.alloc() : memref<16x1024x64xf16>
    %alloc_7 = memref.alloc() : memref<16x1024x64xf32>

    // Initialize Q, K, V with random numbers in range [-1, 1]
    %cast = memref.cast %alloc : memref<16x1024x64xf16> to memref<*xf16>
    %cast_8 = memref.cast %alloc_3 : memref<16x1024x64xf16> to memref<*xf16>
    %cast_9 = memref.cast %alloc_4 : memref<16x1024x64xf16> to memref<*xf16>
    ...
    call @init_3d_memref_to_const_f16(%alloc_5, %cst) : (memref<16x1024x64xf16>, f16) -> ()
    call @init_3d_memref_to_const_f16(%alloc_6, %cst) : (memref<16x1024x64xf16>, f16) -> ()

    // Run GPU version
    %0 = call @gpu_impl(%alloc, %alloc_3, %alloc_4, %alloc_5) : (memref<16x1024x64xf16>, memref<16x1024x64xf16>, memref<16x1024x64xf16>, memref<16x1024x64xf16>) -> memref<16x1024x64xf16>

    // Run CPU reference version
    %1 = call @cpu_impl(%alloc, %alloc_3, %alloc_4, %alloc_6, %cst_0) : (memref<16x1024x64xf16>, memref<16x1024x64xf16>, memref<16x1024x64xf16>, memref<16x1024x64xf16>, f32) -> memref<16x1024x64xf16>

    // Convert CPU output to f32 for comparison
    scf.for %arg0 = %c0 to %c16 step %c1 {
      scf.for %arg1 = %c0 to %c1024 step %c1 {
        scf.for %arg2 = %c0 to %c64 step %c1 {
          %2 = memref.load %alloc_6[%arg0, %arg1, %arg2] : memref<16x1024x64xf16>
          %3 = arith.extf %2 : f16 to f32
          memref.store %3, %alloc_7[%arg0, %arg1, %arg2] : memref<16x1024x64xf32>
        }
      }
    }

    // Compare results
    %cast_10 = memref.cast %0 : memref<16x1024x64xf16> to memref<*xf16>
    %cast_11 = memref.cast %alloc_7 : memref<16x1024x64xf32> to memref<*xf32>
    // CHECK: [ALLCLOSE: TRUE]
    call @printAllcloseF16(%cast_10, %cast_11) : (memref<*xf16>, memref<*xf32>) -> ()

    // Cleanup
    memref.dealloc %alloc : memref<16x1024x64xf16>
    memref.dealloc %alloc_3 : memref<16x1024x64xf16>
    memref.dealloc %alloc_4 : memref<16x1024x64xf16>
    memref.dealloc %alloc_5 : memref<16x1024x64xf16>
    memref.dealloc %alloc_6 : memref<16x1024x64xf16>
    memref.dealloc %alloc_7 : memref<16x1024x64xf32>
    return
  }
