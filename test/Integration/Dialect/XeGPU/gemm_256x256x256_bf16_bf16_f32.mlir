...
module @gemm attributes {gpu.container_module} {
  func.func @test(%A: memref<256x256xbf16>, %B: memref<256x256xbf16>, %C: memref<256x256xf32>) -> memref<256x256xf32> attributes {llvm.emit_c_interface} {
    ...
    %A_gpu = gpu.alloc host_shared () : memref<256x256xbf16>
    memref.copy %A, %A_gpu : memref<256x256xbf16> to memref<256x256xbf16>
    %B_gpu = gpu.alloc host_shared () : memref<256x256xbf16>
    memref.copy %B, %B_gpu : memref<256x256xbf16> to memref<256x256xbf16>
    %C_gpu = gpu.alloc host_shared () : memref<256x256xf32>
    memref.copy %C, %C_gpu : memref<256x256xf32> to memref<256x256xf32>
    gpu.launch_func  @test_kernel::@test_kernel blocks in (%c1, %c1, %c1) threads in (%c8, %c4, %c1) args(%A_gpu : memref<256x256xbf16>, %B_gpu : memref<256x256xbf16>, %C_gpu : memref<256x256xf32>)
    gpu.dealloc  %A_gpu : memref<256x256xbf16>
    gpu.dealloc  %B_gpu : memref<256x256xbf16>
    return %C_gpu : memref<256x256xf32>
  }

  gpu.module @test_kernel attributes {spirv.target_env = #spirv.target_env<#spirv.vce<v1.4, [Addresses, Float16Buffer, Int64, Int16, Int8, Kernel, Linkage, Vector16, GenericPointer, Groups, Float16, Float64, AtomicFloat32AddEXT, ExpectAssumeKHR, SubgroupDispatch, VectorComputeINTEL, VectorAnyINTEL, Bfloat16ConversionINTEL]>, api=OpenCL, #spirv.resource_limits<>>} {
    gpu.func @test_kernel(%A: memref<256x256xbf16>, %B: memref<256x256xbf16>, %C: memref<256x256xf32>) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      ...
      // stage 1
      %A_sg_prefetch_tile_iter0 = xegpu.create_nd_tdesc %A[%A_sg_prefetch_offset_x, %c0] : memref<256x256xbf16> -> !xegpu.tensor_desc<8x32xbf16>
      ...
      %B_sg_prefetch_tile_iter0 = xegpu.create_nd_tdesc %B[%B_sg_prefetch_offset_x, %B_sg_prefetch_offset_y]  : memref<256x256xbf16> -> !xegpu.tensor_desc<8x32xbf16>
      ...
      %A_sg_init_tile_0 = xegpu.create_nd_tdesc %A[%C_sg_tile_offset_x, %c0]  : memref<256x256xbf16> -> !xegpu.tensor_desc<16x16xbf16, #xegpu.block_tdesc_attr<array_length =2>>
      ...
      //create B tiles
      %B_sg_init_tile_0 = xegpu.create_nd_tdesc %B[%c0, %C_sg_tile_offset_y] : memref<256x256xbf16> -> !xegpu.tensor_desc<16x16xbf16, #xegpu.block_tdesc_attr<array_length =2>>
      ...
      %k_loop_result:24 = scf.for %k = %c0 to %c256 step %c32 iter_args (
      ...
      %c_sg_tile_00 = xegpu.create_nd_tdesc %C[%C_sg_tile_offset_x, %C_sg_tile_offset_y] : memref<256x256xf32> -> !xegpu.tensor_desc<8x16xf32>
  ...
  func.func @cpu_reference(%A : memref<256x256xbf16>, %B : memref<256x256xbf16>, %C : memref<256x256xf32>) {
    %c256 = arith.constant 256 : index
    %c16 = arith.constant 16 : index
    %c1 = arith.constant 1 : index
    %c0 = arith.constant 0 : index
      ...
      scf.for %j = %c0 to %c256 step %c1 {
        ...
        %c_val = scf.for %k_tile = %c0 to %c256 step %c16 iter_args(%c_partial = %c_curr) -> f32 {
          %c_val_dpas = scf.for %k = %c0 to %c16 step %c1 iter_args(%c_dpas_partial = %c_partial) -> f32 {
            ...
            %a_val = memref.load %A[%i, %k_dpas] : memref<256x256xbf16>
            %b_val = memref.load %B[%k_dpas, %j] : memref<256x256xbf16>
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %A = memref.alloc() : memref<256x256xbf16>
    ...
    %A_random = memref.cast %A : memref<256x256xbf16> to memref<*xbf16>
    ...
    // }
    // Option 2:  convert the memref to 1D and fill with random values in (0.0, 1.0)
    %B_random = memref.cast %B : memref<256x256xbf16>  to memref<*xbf16>
    ...
    // run GPU
    %2 = call @test(%A, %B, %C) : (memref<256x256xbf16>, memref<256x256xbf16>, memref<256x256xf32>) -> memref<256x256xf32>

    // run CPU
    call @cpu_reference(%A, %B, %C_ref) : (memref<256x256xbf16>, memref<256x256xbf16>, memref<256x256xf32>) -> ()
