...
module @gemm attributes {gpu.container_module} {
  memref.global "private" constant @__constant_8x32xbf16 : memref<8x32xbf16> = dense<0.000000e+00>
  func.func @test(%arg0: memref<8x32xbf16>, %arg1: memref<8x32xbf16>) -> memref<8x32xbf16> attributes {llvm.emit_c_interface} {
    %c1 = arith.constant 1 : index
    %c8 = arith.constant 8 : index
    %memref = gpu.alloc  () : memref<8x32xbf16>
    gpu.memcpy  %memref, %arg0 : memref<8x32xbf16>, memref<8x32xbf16>
    %memref_0 = gpu.alloc  () : memref<8x32xbf16>
    gpu.memcpy  %memref_0, %arg1 : memref<8x32xbf16>, memref<8x32xbf16>
    %memref_1 = gpu.alloc  () : memref<8x32xbf16>
    gpu.launch_func  @test_kernel::@test_kernel blocks in (%c1, %c1, %c1) threads in (%c8, %c1, %c1)  args(%memref : memref<8x32xbf16>, %memref_0 : memref<8x32xbf16>, %memref_1 : memref<8x32xbf16>)
    gpu.dealloc  %memref : memref<8x32xbf16>
    gpu.dealloc  %memref_0 : memref<8x32xbf16>
    %alloc = memref.alloc() : memref<8x32xbf16>
    gpu.memcpy  %alloc, %memref_1 : memref<8x32xbf16>, memref<8x32xbf16>
    gpu.dealloc  %memref_1 : memref<8x32xbf16>
    return %alloc : memref<8x32xbf16>
  }
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %alloc = memref.alloc() : memref<8x32xbf16>
    %cast = memref.cast %alloc : memref<8x32xbf16> to memref<*xbf16>
    call @fillResource1DRandomBF16(%cast, %cst, %cst_0, %false) : (memref<*xbf16>, f32, f32, i1) -> ()
    %alloc_1 = memref.alloc() : memref<8x32xbf16>
    %cast_2 = memref.cast %alloc_1 : memref<8x32xbf16> to memref<*xbf16>
    call @fillResource1DRandomBF16(%cast_2, %cst, %cst_0, %false) : (memref<*xbf16>, f32, f32, i1) -> ()
    %alloc_3 = memref.alloc() : memref<8x32xf32>
    scf.for %arg0 = %c0 to %c8 step %c1 {
      scf.for %arg1 = %c0 to %c32 step %c1 {
        %1 = memref.load %alloc[%arg0, %arg1] : memref<8x32xbf16>
        %2 = memref.load %alloc_1[%arg0, %arg1] : memref<8x32xbf16>
        %3 = arith.extf %1 : bf16 to f32
        %4 = arith.extf %2 : bf16 to f32
        %5 = arith.addf %3, %4 : f32
        %6 = arith.truncf %5 : f32 to bf16
        %7 = arith.extf %6 : bf16 to f32
        memref.store %7, %alloc_3[%arg0, %arg1] : memref<8x32xf32>
      }
    }
    //call @printMemrefBF16(%C_cast) : (memref<*xbf16>) -> ()
    //call @printMemrefF32(%ref_cast) : (memref<*xf32>) -> ()
    // CHECK: [ALLCLOSE: TRUE]
    %0 = call @test(%alloc, %alloc_1) : (memref<8x32xbf16>, memref<8x32xbf16>) -> memref<8x32xbf16>
    %cast_4 = memref.cast %0 : memref<8x32xbf16> to memref<*xbf16>
    %cast_5 = memref.cast %alloc_3 : memref<8x32xf32> to memref<*xf32>
    call @printAllcloseBF16(%cast_4, %cast_5) : (memref<*xbf16>, memref<*xf32>) -> ()
    return
  }
