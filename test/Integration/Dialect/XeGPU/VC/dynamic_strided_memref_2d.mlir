...
module @gemm attributes {gpu.container_module} {
  ...
  func.func @test(%arg0: memref<32x64xf16>, %arg1: memref<32x64xf16>) -> memref<32x64xf32> attributes {llvm.emit_c_interface} {
    ...
    %meref_host = memref.alloc() : memref<32x64xf32>
    %cast_host = memref.cast %meref_host : memref<32x64xf32> to memref<*xf32>
    call @fillResource1DF32(%cast_host, %cst) : (memref<*xf32>, f32) -> ()
    %memref_1 = gpu.alloc  () : memref<32x64xf32>
    gpu.memcpy  %memref_1, %meref_host : memref<32x64xf32>, memref<32x64xf32>
    %subview = memref.subview %memref[%c0, %c0] [%c32, %c32] [%c1, %c1] : memref<32x64xf16> to memref<?x?xf16, strided<[?, ?], offset: ?>>
    %subview_2 = memref.subview %memref_0[%c0, %c0] [%c32, %c32] [%c1, %c1] : memref<32x64xf16> to memref<?x?xf16, strided<[?, ?], offset: ?>>
    %subview_3 = memref.subview %memref_1[%c0, %c0] [%c32, %c32] [%c1, %c1] : memref<32x64xf32> to memref<?x?xf32, strided<[?, ?], offset: ?>>
    gpu.launch_func  @test_kernel::@test_kernel blocks in (%c4, %c2, %c1) threads in (%c1, %c1, %c1)  args(%subview : memref<?x?xf16, strided<[?, ?], offset: ?>>, %subview_2 : memref<?x?xf16, strided<[?, ?], offset: ?>>, %subview_3 : memref<?x?xf32, strided<[?, ?], offset: ?>>, %c32 : index, %c32 : index, %c64 : index, %c1 : index)
    memref.dealloc  %meref_host : memref<32x64xf32>
  ...
  gpu.module @test_kernel  {
    gpu.func @test_kernel(%arg0: memref<?x?xf16, strided<[?, ?], offset: ?>>, %arg1: memref<?x?xf16, strided<[?, ?], offset: ?>>, %arg2: memref<?x?xf32, strided<[?, ?], offset: ?>>, %arg3: index, %arg4: index, %arg5: index, %arg6: index) kernel attributes {VectorComputeFunctionINTEL, gpu.known_block_size = array<i32: 1, 1, 1>, gpu.known_grid_size = array<i32: 4, 2, 1>, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      ...
      %2 = xegpu.create_nd_tdesc %arg2[%0, %1], shape : [%arg3, %arg4], strides : [%arg5, %arg6] : memref<?x?xf32, strided<[?, ?], offset: ?>> -> !xegpu.tensor_desc<8x16xf32>
      %3 = xegpu.load_nd %2  : !xegpu.tensor_desc<8x16xf32> -> vector<8x16xf32>
      %4 = scf.for %arg7 = %c0 to %c32 step %c16 iter_args(%arg8 = %3) -> (vector<8x16xf32>) {
        %5 = xegpu.create_nd_tdesc %arg0[%0, %arg7], shape : [%arg3, %arg4], strides : [%arg5, %arg6] : memref<?x?xf16, strided<[?, ?], offset: ?>> -> !xegpu.tensor_desc<8x16xf16>
        %6 = xegpu.load_nd %5  : !xegpu.tensor_desc<8x16xf16> -> vector<8x16xf16>
        %7 = xegpu.create_nd_tdesc %arg1[%arg7, %1], shape : [%arg3, %arg4], strides : [%arg5, %arg6] : memref<?x?xf16, strided<[?, ?], offset: ?>> -> !xegpu.tensor_desc<16x16xf16>
        %8 = xegpu.load_nd %7 <{packed}> : !xegpu.tensor_desc<16x16xf16> -> vector<8x16x2xf16>
        %9 = arith.addf %6, %cst : vector<8x16xf16>
        %10 = xegpu.dpas %9, %8, %arg8 : vector<8x16xf16>, vector<8x16x2xf16>, vector<8x16xf32> -> vector<8x16xf32>
