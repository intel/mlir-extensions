...
module @gemm attributes {gpu.container_module} {
  func.func @test(%A: memref<1024x1024xf16>, %B: memref<1024x1024xf16>, %C: memref<1024x1024xf32>, %bcast: memref<1x1024xf16>) -> memref<1024x1024xf32> attributes {llvm.emit_c_interface} {
    ...
    %bcast_gpu = gpu.alloc  host_shared () : memref<1x1024xf16>
    memref.copy %bcast, %bcast_gpu : memref<1x1024xf16> to memref<1x1024xf16>
    gpu.launch_func  @test_kernel::@test_kernel blocks in (%c64, %c32, %c1) threads in (%c1, %c1, %c1) args(%A_gpu : memref<1024x1024xf16>, %B_gpu : memref<1024x1024xf16>, %C_gpu : memref<1024x1024xf32>, %bcast_gpu :  memref<1x1024xf16>)
  ...
  gpu.module @test_kernel attributes {spirv.target_env = #spirv.target_env<#spirv.vce<v1.4, [Addresses, Float16Buffer, Int64, Int16, Int8, Kernel, Linkage, Vector16, GenericPointer, Groups, Float16, Float64, AtomicFloat32AddEXT, ExpectAssumeKHR, SubgroupDispatch, VectorComputeINTEL, VectorAnyINTEL], [SPV_EXT_shader_atomic_float_add, SPV_KHR_expect_assume, SPV_INTEL_vector_compute]>, api=OpenCL, #spirv.resource_limits<>>} {
    gpu.func @test_kernel(%A: memref<1024x1024xf16>, %B: memref<1024x1024xf16>, %C: memref<1024x1024xf32>, %bcast:  memref<1x1024xf16>) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      ...
      %bcast_init_tile = xetile.init_tile %bcast[%c0, %c0] : memref<1x1024xf16> -> !xetile.tile<1x32xf16>
      ...
      %out:4 = scf.for %k = %c0 to %c1024 step %c32
        ...
        -> (!xetile.tile<16x32xf16>, !xetile.tile<32x32xf16>, !xetile.tile<1x32xf16>, vector<16x32xf32>) {

        // load A and B tiles
        %a = xetile.load_tile %a_tile  : !xetile.tile<16x32xf16> -> vector<16x32xf16>
        %b = xetile.load_tile %b_tile  : !xetile.tile<32x32xf16> -> vector<32x32xf16>
        %bcast_val = xetile.load_tile %bcast_tile  : !xetile.tile<1x32xf16> -> vector<1x32xf16>
        %t1 = vector.shape_cast %bcast_val : vector<1x32xf16> to vector<32xf16>
        %t2 = vector.shape_cast %t1 : vector<32xf16> to vector<32x1xf16>
        // broadcast and add to a
        %bcast_val_ = xetile.broadcast %t2 [1] : vector<32x1xf16> -> vector<32x32xf16>
        %b_value = arith.addf %b, %bcast_val_ : vector<32x32xf16>
        // perform dpas and accumulate
        %c_new_value = xetile.tile_mma %a, %b_value, %c_value
        ...
        %bcast_next_tile = xetile.update_tile_offset %bcast_tile, [%c0, %c32]
          : !xetile.tile<1x32xf16>, index, index -> !xetile.tile<1x32xf16>
        // partial C tile result
        scf.yield %a_next_tile, %b_next_tile, %bcast_next_tile, %c_new_value
          : !xetile.tile<16x32xf16>, !xetile.tile<32x32xf16>, !xetile.tile<1x32xf16>, vector<16x32xf32>
      }
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %A_random = memref.cast %A : memref<1024x1024xf16> to memref<*xf16>
    call @fillResource1DRandomF16(%A_random, %cf_lower, %cf_upper, %c_gen_int) : (memref<*xf16>, f32, f32, i1) -> ()
    %B_random = memref.cast %B : memref<1024x1024xf16> to memref<*xf16>
    call @fillResource1DRandomF16(%B_random, %cf_lower, %cf_upper, %c_gen_int) : (memref<*xf16>, f32, f32, i1) -> ()
    %bcast_random = memref.cast %bcast : memref<1x1024xf16> to memref<*xf16>
    call @fillResource1DRandomF16(%bcast_random, %cf_lower, %cf_upper, %c_gen_int) : (memref<*xf16>, f32, f32, i1) -> ()
    ...
    scf.for %i = %c0 to %c1024 step %c1 {
      scf.for %j = %c0 to %c1024 step %c1 {
        ...
        %c_val = scf.for %k = %c0 to %c1024 step %c1 iter_args(%c_partial = %c_curr) -> f32 {
          %a = memref.load %A[%i, %k] : memref<1024x1024xf16>
          %b = memref.load %B[%k, %j] : memref<1024x1024xf16>
          %bcast_val = memref.load %bcast[%c0, %k] : memref<1x1024xf16>
          %b_val = arith.addf %b, %bcast_val : f16
          %t = arith.mulf %a, %b_val : f16
    ...
    %2 = call @test(%A, %B, %C, %bcast) : (memref<1024x1024xf16>, memref<1024x1024xf16>, memref<1024x1024xf32>, memref<1x1024xf16>) -> memref<1024x1024xf32>
