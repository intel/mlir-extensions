...
module @gemm attributes {gpu.container_module} {
  func.func @test(%A: memref<1024x1024xf16>, %B: memref<1024x1024xf16>, %C: memref<1024x1024xf32>) -> memref<1024x1024xf32> attributes {llvm.emit_c_interface} {
    ...
    %A_gpu_cast = memref.cast %A_gpu : memref<1024x1024xf16> to memref<?x?xf16>
    %B_gpu_cast = memref.cast %B_gpu : memref<1024x1024xf16> to memref<?x?xf16>
    %C_gpu_cast = memref.cast %C_gpu : memref<1024x1024xf32> to memref<?x?xf32>

    gpu.launch_func  @test_kernel::@test_kernel blocks in (%c64, %c32, %c1) threads in (%c1, %c1, %c1) args(%A_gpu_cast : memref<?x?xf16>, %B_gpu_cast : memref<?x?xf16>, %C_gpu_cast : memref<?x?xf32>)
  ...
  gpu.module @test_kernel attributes {spirv.target_env = #spirv.target_env<#spirv.vce<v1.4, [Addresses, Float16Buffer, Int64, Int16, Int8, Kernel, Linkage, Vector16, GenericPointer, Groups, Float16, Float64, AtomicFloat32AddEXT, ExpectAssumeKHR, SubgroupDispatch, VectorComputeINTEL, VectorAnyINTEL], [SPV_EXT_shader_atomic_float_add, SPV_KHR_expect_assume, SPV_INTEL_vector_compute]>, api=OpenCL, #spirv.resource_limits<>>} {
    gpu.func @test_kernel(%A: memref<?x?xf16>, %B: memref<?x?xf16>, %C: memref<?x?xf32>) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      ...
      %c_init_tile = xetile.init_tile %C[%m, %n], [%c1024, %c1024], [%c1024, %c1] : memref<?x?xf32> -> !xetile.tile<16x32xf32>
      %c_init_value = xetile.load_tile %c_init_tile  : !xetile.tile<16x32xf32> -> vector<16x32xf32>
      // initalize A and B tiles
      %a_init_tile = xetile.init_tile %A[%m, %c0], [%c1024, %c1024], [%c1024, %c1] : memref<?x?xf16> -> !xetile.tile<16x32xf16>
      %b_init_tile = xetile.init_tile %B[%c0, %n], [%c1024, %c1024], [%c1024, %c1] : memref<?x?xf16> -> !xetile.tile<32x32xf16>
