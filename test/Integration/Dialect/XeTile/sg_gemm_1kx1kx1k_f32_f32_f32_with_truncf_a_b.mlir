...
module @gemm attributes {gpu.container_module} {
  func.func @test(%A: memref<1024x1024xf32>, %B: memref<1024x1024xf32>, %C: memref<1024x1024xf32>) -> memref<1024x1024xf32> attributes {llvm.emit_c_interface} {
    ...
    memref.copy %B, %B_gpu : memref<1024x1024xf32> to memref<1024x1024xf32>
    %C_gpu = gpu.alloc  host_shared () : memref<1024x1024xf32>
    memref.copy %C, %C_gpu : memref<1024x1024xf32> to memref<1024x1024xf32>
    gpu.launch_func  @test_kernel::@test_kernel blocks in (%c64, %c32, %c1) threads in (%c1, %c1, %c1) args(%A_gpu : memref<1024x1024xf32>, %B_gpu : memref<1024x1024xf32>, %C_gpu : memref<1024x1024xf32>)
    gpu.dealloc  %A_gpu : memref<1024x1024xf32>
    gpu.dealloc  %B_gpu : memref<1024x1024xf32>
    return %C_gpu : memref<1024x1024xf32>
  }
  gpu.module @test_kernel attributes {spirv.target_env = #spirv.target_env<#spirv.vce<v1.4, [Addresses, Float16Buffer, Int64, Int16, Int8, Kernel, Linkage, Vector16, GenericPointer, Groups, Float16, Float64, AtomicFloat32AddEXT, ExpectAssumeKHR, SubgroupDispatch, VectorComputeINTEL, VectorAnyINTEL], [SPV_EXT_shader_atomic_float_add, SPV_KHR_expect_assume, SPV_INTEL_vector_compute]>, api=OpenCL, #spirv.resource_limits<>>} {
    gpu.func @test_kernel(%A: memref<1024x1024xf32>, %B: memref<1024x1024xf32>, %C: memref<1024x1024xf32>) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      ...
      %a_init_tile = xetile.init_tile %A[%m, %c0] : memref<1024x1024xf32> -> !xetile.tile<16x32xf32>
      %b_init_tile = xetile.init_tile %B[%c0, %n] : memref<1024x1024xf32> -> !xetile.tile<32x32xf32>
      ...
      %out:3 = scf.for %k = %c0 to %c1024 step %c32
        ...
        // load A and B tiles
        %a_value = xetile.load_tile %a_tile  : !xetile.tile<16x32xf32> -> vector<16x32xf32>
        %b_value = xetile.load_tile %b_tile  : !xetile.tile<32x32xf32> -> vector<32x32xf32>
        %a_trunc = arith.truncf %a_value: vector<16x32xf32> to vector<16x32xf16>
        %b_trunc = arith.truncf %b_value: vector<32x32xf32> to vector<32x32xf16>
  ...
  func.func @main() attributes {llvm.emit_c_interface} {
    ...
    %c1 = arith.constant 1 : index
    %c1024 = arith.constant 1024 : index
    %cf_0 = arith.constant 0.0 : f32
    %cf_1 = arith.constant 1.0 : f32
    ...
    scf.for %i = %c0 to %c1024 step %c1 {
      scf.for %j = %c0 to %c1024 step %c1 {
        %t = index.castu %j : index to i32
        %val = arith.uitofp %t : i32 to f32
        memref.store %val, %A[%i, %j] : memref<1024x1024xf32>
      }
    }
    ...
    scf.for %i = %c0 to %c1024 step %c1 {
      scf.for %j = %c0 to %c1024 step %c1 {
        ...
        scf.if %i_j_same {
          memref.store %cf_1, %B[%i, %j] : memref<1024x1024xf32>
        } else {
          memref.store %cf_0, %B[%i, %j] : memref<1024x1024xf32>
        }
      }
    }
    ...
    scf.for %i = %c0 to %c1024 step %c1 {
      scf.for %j = %c0 to %c1024 step %c1 {
        ...
        %c_val = scf.for %k = %c0 to %c1024 step %c1 iter_args(%c_partial = %c_curr) -> f32 {
          %a_val = memref.load %A[%i, %k] : memref<1024x1024xf32>
          %b_val = memref.load %B[%k, %j] : memref<1024x1024xf32>
          %t = arith.mulf %a_val, %b_val : f32
    ...
    %2 = call @test(%A, %B, %C) : (memref<1024x1024xf32>, memref<1024x1024xf32>, memref<1024x1024xf32>) -> memref<1024x1024xf32>
