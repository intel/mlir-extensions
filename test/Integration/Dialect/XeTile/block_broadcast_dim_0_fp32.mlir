...
module @softmax attributes {gpu.container_module} {
  func.func @broadcast_test() -> memref<1024x1024xf32> attributes {llvm.emit_c_interface} {
    ...
    // the kernel is a 16x32 block broadcast. each thread is assigned with a 16x32 block, and broadcast value from vector<1x32xf32> to vector<16x32xf32> along dim-0 independently.
  gpu.module @kernel  {
    gpu.func @softmax_dim_0(%arg0: memref<1024x1024xf32>) kernel attributes {VectorComputeFunctionINTEL, spirv.entry_point_abi = #spirv.entry_point_abi<>} {
      ...
      %cst = arith.constant dense<3.000000e+00> : vector<1x32xf32>
      %2 = xetile.broadcast %cst [0] : vector<1x32xf32> -> vector<16x32xf32>
