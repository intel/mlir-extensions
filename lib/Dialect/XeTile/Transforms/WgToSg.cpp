...
public:
  ...
  matchAndRewrite(xetile::InitTileOp op, OneToNOpAdaptor adaptor,
    ...
    auto createIndexConstant = [&](Type type, int64_t value) {
    ...
    // get the subgroup Id
    auto sgID = rewriter.create<gpu::SubgroupIdOp>(loc, nullptr);
    ...
    // col =  i % cols
    auto sgIdY =
        rewriter.create<index::DivUOp>(loc, sgID, sgLayoutDimYConst);
    auto sgIdX =
        rewriter.create<index::RemUOp>(loc, sgID, sgLayoutDimYConst);
    ...
    SmallVector<OpFoldResult> globalOffsetsX; // cols
    SmallVector<OpFoldResult> globalOffsetsY; // rows
    SmallVector<SmallVector<OpFoldResult>> offsetPermutations;
...
class WGToSGLoadTileOpPattern : public OpConversionPattern<xetile::LoadTileOp> {
  ...
  matchAndRewrite(xetile::LoadTileOp op, OneToNOpAdaptor adaptor,
    ...
    llvm::SmallVector<Value> newLoadOps;
    ...
    for (auto src : sources) {
      ...
      newResultTypes.push_back(newLoadOp.getType());
    }
...
public:
  ...
  void runOnOperation() override {
    ...
    target.addDynamicallyLegalOp<arith::ConstantOp, arith::AddFOp,
                                 math::ExpOp, math::SqrtOp, arith::ExtFOp,
                                 arith::ExtSIOp, arith::ExtUIOp, arith::FPToSIOp,
                                 arith::FPToUIOp, arith::UIToFPOp, arith::SIToFPOp,
                                 arith::TruncFOp, arith::TruncIOp, arith::CmpIOp,
                                 arith::CmpFOp,  arith::IndexCastUIOp,
                                 arith::IndexCastOp, arith::BitcastOp, vector::TransposeOp,
                                 vector::BroadcastOp, vector::MultiDimReductionOp,
                                 vector::ShapeCastOp>(
        [&](Operation *op) -> bool {
          auto mapAttr = llvm::dyn_cast_or_null<xetile::WorkGroupMapAttr>(
              op->getAttr("map"));
