...
static mlir::LogicalResult parseSgMapAttrElements(mlir::AsmParser &parser,
  ...
  if (mmaBlockSize.size() != 2) {
    parser.emitError(loc, "failed to parse SgMapAttr: missing mma_block_size which is to be a `llvm::ArrayRef<unsigned>` with size 2");
    return mlir::failure();
  }
  if (layout.size() != 2) {
    parser.emitError(loc, "failed to parse SgMapAttr: missing wi_layout which is to be a `llvm::ArrayRef<unsigned>` with size 2");
    return mlir::failure();
  }
  if (data.size() != 2) {
    parser.emitError(loc, "failed to parse SgMapAttr: missing wi_data which is to be a `llvm::ArrayRef<unsigned>` with size 2");
    return mlir::failure();
  }
  return mlir::success();
}
...
static mlir::LogicalResult parseWgMapAttrElements(mlir::AsmParser &parser,
                                          llvm::SmallVector<unsigned> &layout,
                                          llvm::SmallVector<unsigned> &data) {
  auto loc = parser.getCurrentLocation();
  auto parseElt = [&]() -> mlir::LogicalResult {
    return mlir::AsmParser::KeywordSwitch<mlir::LogicalResult>(parser)
      .Case("sg_layout", [&](llvm::StringRef, llvm::SMLoc) {
        return parseArrayList(parser, layout, true);
      })
      .Case("sg_data", [&](llvm::StringRef, llvm::SMLoc) {
        return parseArrayList(parser, data, true);
      })
      .Default([&](llvm::StringRef keyword, llvm::SMLoc) {
        return mlir::failure();
  ...
  if (parser.parseRBrace()) return mlir::failure();
  if (layout.size() != 2) {
    parser.emitError(loc, "failed to parse WgMapAttr: missing sg_layout which is to be a `llvm::ArrayRef<unsigned>` with size 2");
    return mlir::failure();
  }
  if (data.size() != 2) {
    parser.emitError(loc, "failed to parse WgMapAttr: missing sg_data which is to be a `llvm::ArrayRef<unsigned>` with size 2");
    return mlir::failure();
  }
  return mlir::success();
}
...
mlir::Attribute XeMapAttr::parse(mlir::AsmParser &parser, mlir::Type type) {
    ...
    mlir::OptionalParseResult result = mlir::AsmParser::KeywordSwitch<mlir::OptionalParseResult>(parser)
      .Case("sg", [&](llvm::StringRef, llvm::SMLoc) {
        ...
        if (mlir::failed(parseSgMapAttrElements(parser, mmaBlockSize, wiLayout, wiData)))
          return mlir::failure();
        sg = imex::xegpu::SgMapAttr::get(parser.getContext(), mmaBlockSize, wiLayout, wiData);
  ...
  if (parser.parseCommaSeparatedList(parseElt)) return {};

  // Parse literal '>'
  if (parser.parseGreater()) return {};

  if(!wg && !sg) {
    parser.emitError(parser.getCurrentLocation(), "Expecting at least one of sg and wg attributes.\n");
    return {};
  }
