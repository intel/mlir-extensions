...
mlir::LogicalResult ptensor::ARangeLowering::matchAndRewrite(
    ::ptensor::ARangeOp op, ::ptensor::ARangeOp::Adaptor adaptor,
  ...
  if (stop.getType() != ityp) {
    stop = rewriter.create<::mlir::UnrealizedConversionCastOp>(loc, ityp, stop)
               .getResult(0);
  }
  if (step.getType() != ityp) {
    step = rewriter.create<::mlir::UnrealizedConversionCastOp>(loc, ityp, step)
               .getResult(0);
  }
  ...
  // create shape vector
  auto ttyp =
      converter.convertType(op.getType()).dyn_cast<::mlir::RankedTensorType>();
  assert(ttyp);
...
// linalg::genericop)
mlir::LogicalResult ptensor::EWBinOpLowering::matchAndRewrite(
    ::ptensor::EWBinOp op, ::ptensor::EWBinOp::Adaptor adaptor,
    ::mlir::ConversionPatternRewriter &rewriter) const {
  ...
  // operation itself we assume this had beeen taken care of and simply use the
  // op's converted type
  ...
  // the shape is not statically known, we need to retrieve it (it's the same as
  // the input shapes)
  // FIXME shape broadcasting: input tensors might have compatible but different
  // shapes
...
::mlir::LogicalResult ptensor::ReductionOpLowering::matchAndRewrite(
    ::ptensor::ReductionOp op, ::ptensor::ReductionOp::Adaptor adaptor,
    ::mlir::ConversionPatternRewriter &rewriter) const {
  // we expect RankedTensorType as operands
  ...
  auto shape = rewriter.create<::mlir::shape::ShapeOfOp>(loc, tmp_tnsr);
  auto tnsr_id =
      initDTensor(loc, rewriter, orginptyp.getDist(), rank, shape, sltyp, lshp);
  ...
  const ::ptensor::ReduceOpId ropid =
      (::ptensor::ReduceOpId)adaptor.op().cast<::mlir::IntegerAttr>().getInt();
  ...
  auto rval = rewriter.create<::mlir::tensor::ExtractOp>(loc, sltyp, rtnsr,
                                                         ::mlir::ValueRange());
