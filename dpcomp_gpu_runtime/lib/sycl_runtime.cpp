...
struct Queue {
  ...
  void launchKernel(sycl::kernel *kernel, size_t gridX, size_t gridY,
                    ...
                    size_t eventIndex, sycl::event &event) {
...
dpcompGpuCreateContext(sycl::device device) {
  return catchAll([&]() {
...
dpcompGpuStreamCreate(sycl::context context) {
  return catchAll([&]() { return new Queue(context); });
}

extern "C" DPCOMP_GPU_RUNTIME_EXPORT void dpcompGpuStreamDestroy(void *queue) {
  catchAll([&]() { delete static_cast<Queue *>(queue); });
}

extern "C" DPCOMP_GPU_RUNTIME_EXPORT void
dpcompGpuAlloc(void *queue, size_t size, size_t alignment, int shared,
               void *events, size_t eventIndex, AllocResult *ret) {
  catchAll([&]() {
    void *mem_ptr;
    if (shared) {
      mem_ptr =
          sycl::malloc_shared(size, static_cast<Queue *>(queue)->sycl_queue);
    } else {
      mem_ptr =
          sycl::malloc_device(size, static_cast<Queue *>(queue)->sycl_queue);
    }
    *ret = AllocResult{mem_ptr, mem_ptr, nullptr};
  });
}

extern "C" DPCOMP_GPU_RUNTIME_EXPORT void *
dpcompGpuModuleLoad(void *queue, const void *data, size_t dataSize) {
  return catchAll([&]() {
...
extern "C" DPCOMP_GPU_RUNTIME_EXPORT void *
dpcompGpuKernelGet(void *queue, void *module, const char *name) {
  return catchAll([&]() {
    sycl::kernel *kernel;
...
extern "C" DPCOMP_GPU_RUNTIME_EXPORT void *
dpcompGpuLaunchKernel(void *queue, void *kernel, size_t gridX, size_t gridY,
                      size_t gridZ, size_t blockX, size_t blockY, size_t blockZ,
  ...
  return catchAll([&]() {
    ...
    static_cast<Queue *>(queue)->launchKernel(
        ...
        static_cast<ParamDesc *>(params), eventIndex, *event);
    return event;
  });
}
...
extern "C" DPCOMP_GPU_RUNTIME_EXPORT void dpcompGpuWait(void *event) {
  ...
  catchAll([&]() { static_cast<sycl::event *>(event)->wait(); });
}

extern "C" DPCOMP_GPU_RUNTIME_EXPORT void dpcompGpuDeAlloc(void *queue,
                                                           void *ptr) {
  catchAll([&]() {});
}