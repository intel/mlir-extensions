...
def _configure_llvm_build(
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not create build directory {llvm_build_dir}",
            llvm_build_dir,
        )
...
def _get_llvm(llvm_sha=None, working_dir=None):
    ...
    then the commit is checked out into a new branch. The location of the
    cloned git repo is returned to caller.
    ...
    Returns:
        str: Path to the "llvm-project" sub directory inside the cloned
        directory.
    """
